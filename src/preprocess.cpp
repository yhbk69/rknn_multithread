...
/**
 * Letterbox图像预处理: 保持宽高比的缩放+填充
 * 将图像等比例缩放到目标尺寸，不足的部分用指定颜色填充
 * 这样可以避免图像变形，保持目标原始比例
 *
 * @param image 原始输入图像
 * @param padded_image 输出: 填充后的图像
 * @param pads 输出: 四个方向的填充像素数
 * @param scale 缩放比例
 * @param target_size 目标尺寸(如640x640)
 * @param pad_color 填充颜色(默认灰色)
 */
...
/**
 * 使用RGA硬件加速进行图像缩放
 * RGA是Rockchip芯片的2D图形加速单元，比CPU缩放快很多
 *
 * @param src 输出: RGA源图像缓冲区
 * @param dst 输出: RGA目标图像缓冲区
 * @param image 原始输入图像
 * @param resized_image 输出: 缩放后的图像
